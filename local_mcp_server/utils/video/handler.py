...
class _FrameTimestamp:
    def __init__(self, frame_index: int, timestamp: float):
        self.frame_index = frame_index
        self.timestamp = timestamp

class VideoHandler:
    _frame_timestamp_cache: dict[str, list[_FrameTimestamp]] = {}
    def __init__(self, params: VideoHandlerParams):
        ...
        self._video_path = os.path.join(self._video_dir_path, self._video_name)
    ...
    async def summarize_video(self) -> str:
        await self._download_video()
        summarizer = VideoSummarizer(flow_id=str(self._params.flow_id))
    ...
    def _extract_frame_buffer(self, video_sec) -> _FrameInfo:
        self._load_frame_timestamps()

        # Find the best matching frame for the requested timestamp
        best_frame_index = self._find_closest_frame(video_sec, self._frame_timestamp_cache[self._video_path])
    ...
    def _load_frame_timestamps(self) -> list[_FrameTimestamp]:
        """Load all frame timestamps by iterating through video. Returns list of (frame_index, timestamp)."""
        if self._video_path in self._frame_timestamp_cache:
            return self._frame_timestamp_cache[self._video_path]
        try:
            cap = cv2.VideoCapture(self._video_path)
            timestamps = []
            frame_idx = 0
            ...
            while True:
                ...
                frame_ts = _FrameTimestamp(frame_idx, ts)
            ...
            self._frame_timestamp_cache[self._video_path] = timestamps
            return timestamps
        except Exception as e:
            raise RuntimeError(f"Failed to load frame timestamps: {e}")

    def _find_closest_frame(self, target_timestamp: float, frame_timestamps: list[_FrameTimestamp]) -> int:
        ...
        last_timestamp = frame_timestamps[-1].timestamp
        ...
        best_idx = 0
        min_diff = abs(frame_timestamps[0].timestamp - target_timestamp)

        for frame_ts in frame_timestamps:
            diff = abs(frame_ts.timestamp - target_timestamp)
            if diff < min_diff:
                min_diff = diff
                best_idx = frame_ts.frame_index
            elif diff > min_diff:
