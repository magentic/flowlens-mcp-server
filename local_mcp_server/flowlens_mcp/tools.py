...
@server_instance.flowlens_mcp.tool
async def summarize_flow_video(flow_id: int, ctx: Context) -> str:
    """
    Summarize the video recording of a specific flow using Google Generative AI.
    The summary is generated by analyzing the video content and extracting key events and information.
    Favour this tool if get_flow_full_comments() doesn't provide enough context about what happened in the flow.
    
    Args:
        flow_id (int): The ID of the flow to summarize the video for.
    Returns:
        str: The summary of the video content. Each line is a key event with a timestamp in seconds. the line format is:
                [timestamp_in_seconds:int] - [event_description:str]
                e.g. 00:13 - The user clicks the "Save" button to apply the changes.
                
    """
    service: flow_lens.FlowLensService = ctx.get_state("flowlens_service")
    return await service.summarize_video(flow_id)
